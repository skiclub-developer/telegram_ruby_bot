...
def punish_member(bot, message)
  ...
  response = JSON.parse(Faraday.post("http://localhost:3000/api/v1/members/punish", body).body, object_class: OpenStruct)
...
def lost_game_players(bot, message)
  response = Faraday.patch("http://localhost:3000/api/v1/members")
  body = {
      "telegram_user": telegram_user(message)
  }
  payment_response = Faraday.post("http://localhost:3000/api/v1/member_penalties", body)
...
def add_penalty(bot, message, type)
  ...
  response = JSON.parse(Faraday.patch("http://localhost:3000/api/v1/members", body).body, object_class: OpenStruct)
  response.updated_members.each do |updated_member|
    if type == "beer"
