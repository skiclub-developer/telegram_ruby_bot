...
def show_member_penalties(bot, message)
  ...
  response = JSON.parse(Faraday.get("http://localhost:3000/api/v1/members?members=#{members_to_find}").body, object_class: OpenStruct)
...
def list_members(bot, message)
  response = JSON.parse(Faraday.get("http://localhost:3000/api/v1/members").body, object_class: OpenStruct)
  members = []
...
def list_member_payments(bot, message)
  ...
  if member_to_find != ""
    response = JSON.parse(Faraday.get("http://localhost:3000/api/v1/members?member=#{member_to_find}").body, object_class: OpenStruct)
...
def list_all_member_penalties(bot, message)
  member_to_find = message.text.partition(' ').last
  if member_to_find != ""
    response = JSON.parse(Faraday.get("http://localhost:3000/api/v1/member_penalties?name=#{member_to_find}").body, object_class: OpenStruct)
